...
\startTestCase[should fail if not a list]
\startCTest
...
\startTestCase[should succeed if a list]
\startCTest
  ...
  AssertIntEquals(asParserTextPosition(aCtx), 23);
  AssertIntFalse(asParserFailed(aCtx));
  AssertIntFalse(asParserCommitted(aCtx));
  AssertPtrNotNull(asCtxData(aCtx));
  AssertPtrNull(asCtxProcess(aCtx));
  ...
  AssertIntEquals(asParserTextPosition(aCtx), 28);
  AssertIntFalse(asParserFailed(aCtx));
  AssertIntFalse(asParserCommitted(aCtx));
  AssertPtrNotNull(asCtxData(aCtx));
  AssertPtrNull(asCtxProcess(aCtx));
...
\startTestSuite[parseListClosingWord]
...
\startCHeader
void parseListClosingWord(ContextObj *aCtx);
\stopCHeader
...
\startTestCase[should not committed but failed if not a list closing]
\startCTest
  parseString(aCtx, "this is a non-list test");
  AssertIntFalse(asParserFailed(aCtx));
  AssertIntFalse(asParserCommitted(aCtx));
...
\startTestCase[should be committed and failed if a list closing]
\startCTest
