...
\startCCode
static int lua_rules_getGitVersion (lua_State *lstate) {
...
static const struct luaL_Reg lua_rules [] = {
  {"gitVersion", lua_rules_getGitVersion},
  {NULL, NULL}
};

int luaopen_joylol_rules (lua_State *lstate) {
  getJoyLoLInterpInto(lstate, jInterp);
  registerRules(jInterp);
  luaL_newlib(lstate, lua_rules);
  return 1;
}
\stopCCode
...
process. The following \type{requireStaticallyLinkedRules} does 
just this. 

\startCHeader
Boolean requireStaticallyLinkedRules(
  lua_State *lstate
);
\stopCHeader
...
Boolean requireStaticallyLinkedRules(
  ...
  luaopen_joylol_rules(lstate);
  lua_setfield(lstate, -2, "joylol.rules");
