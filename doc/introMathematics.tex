...
Over the next three definitions, below, we will explore what a wee beastie 
can do, know, and identify. These three definitions are of necessity, 
...
language \type{joylol} which is in turn loosely based upon Manfred von 
Thun's \emph{concatenative} language, \type{joy}\footnote{To better 
understand Manfred von Thun's programming language, \type{joy}, see 
...
\emph{concatenative} language was \type{Forth}. See 
\cite{brodie1987StartingForth} and \cite{brodie2004thinkingForth} for an 
introduction to \type{Forth}.}. While most programming languages are 
applicative and make extensive use of variables, we will use the 
semantically simpler concatenative language. In classical terms the 
category of computations of an applicative functional languages, such as 
\type{Haskell}, corresponds to a Topos, and that of a concatenative 
functional languages, such as \type{joylol} or \type{joy} assumes no more 
structure than that of a Category. Not surprisingly, we will, eventually, 
show that the compuational power of \type{joylol} is equivalent to that of 
any other programming language. In classical terms this means that the 
Category of \type{joylol}'s computations is actually has the structure of 
a Topos (or more correctly a Topos-CoTopos pair). 
...
\item Constructive actions

\startitemize[n]

\item create a new \quote{top} \quote{\type{nil}} \lol.

\item create a new \quote{top} \lol\ by removing the \quote{top} two 
\lols\ and \quote{\type{cons}}ing them together.

\stopitemize

\item Destructive actions

\startitemize[n]

\item delete the \quote{top} \lol\ by \quote{\type{pop}}ing it off the 
stack. 

\item create two new \quote{top} \lols\ by removing the \quote{top} 
\lol\ and replacing it with both the \quote{\type{car}} and 
\quote{\type{cdr}} of the original \lol. 
...
\item \quote{\type{dup}}licate the \quote{top} \lol.

\item \quote{\type{swap}} the \quote{order} of the \quote{top} two \lols. 
