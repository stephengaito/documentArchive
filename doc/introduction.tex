...
\item determine if an existing \lol\ is \quote{\type{nil}}.
...
beastie can not know if it is well-founded or not. \stopMMundi

\subsection[title=Identity and Bisimulation]

When are two \quote{widgets} or \lol's the \emph{same} \quote{widget} or 
\lol? Given the capabilities listed above, no wee beastie can know 
\emph{identity}. The best a wee beastie can do is to compare a pair of 
\lol's using the same collection of \emph{tests}. 
