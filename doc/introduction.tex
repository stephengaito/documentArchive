...
\MMundi{extra-foundational commentary} using classical mathematics, 
typically, classical Category theory, and the actual re-founded 
foundations. We will do this by placing these \MMundi{extra-foundational 
comments} in the colour \color[blue]{blue} and in the {\sans Sans-Serif 
font}. 

\startMMundi From the beginning of the next subsection, all 
extra-foundational comments will be carefully delineated from the 
re-foundations themselves. \stopMMundi 
...
wee beastie can \emph{do}, define a \emph{endo-functor} from the Category 
of Sets to itself\footnote{See, for example, \cite{jacobs2017coalgebras}}. 
The initial (final) Algebra (Co-Algebra) associated to this endo-functor 
is the collection of well-founded (non-well-founded) Lists (or more 
correctly Lists of Lists). Notice our \emph{explicit} use of John 
McCarthy's notation for the programming language LISP, 
...
well-founded. However, given any \emph{existing} \lol, a wee beastie can 
not know if it is well-founded or not. Moreover, with the capabilities 
listed above, a wee beastie can not \emph{know}, apriori, if two \lol s 
are \emph{equal}. The best a wee beastie can know, is that a pair of \lol 
s have \quote{passed} all of the same collection of \quote{tests}. This is 
the concept of Bisimulation, see Chapter 3 of \cite{jacobs2017coalgebras}. 
\stopMMundi 
