...
be defined by their use in our theory. In the following definition, we 
\emph{could} use the word \quote{widget} to emphasize, with Hilbert, that 
our undefined terms can represent anything which behaves in the prescribed 
way. Having made this point, we will actually use the undefinded but 
slightly more suggestive terminology of \quote{Lists of Lists} or \lols. 
...
Mathematician can do to \lols\ in its environment.

\stopDefinition

\startMMundi From a classical point of view, these four things that a 
...
the collection of \emph{non-well-founded} Lists of Lists. 

For a given ordinal, $\lambda$, the concept of 
\quote{$\lambda$-computational power}, is critical to our theory. Once we 
have progressed far enough to be able to define the ordinals, \emph{will} 
define $\lambda$-computation as the computing \quote{power} associated 
with a wee beastie whose computational \quote{traces} are at most 
$\lambda$ in length. 

Assuming a wee beastie has $\omega$-computational power, then clearly any 
\lol\ the wee beastie might \emph{create} must be well-founded. However, 
given any already \emph{existing} \lol, a wee beastie can not know if it 
is well-founded or not. Hence, for our work, the collection of 
\emph{non-well-founded} \lols\ is the \emph{primary} collection. The 
sub-collection of \emph{well-founded} \lols, is important but secondary. 
...
\item the test which succeeds if two other tests both succeed.

\item the test which succeeds if one or more of two other tests succeeds. 

\item the test which succeeds if another test fails.

\item the test which succeeds if another test succeeds after 
...
\item the test which succeeds if another tests succeeds after 
creating a new \quote{\type{nil}} \lol.

\stopitemize

If any of the above tests does \emph{not} succeed, then that 
test fails. This is \emph{all} that any wee beastie can know 
by testing \lols\ in its environment.
...
\startMMundi Moreover, with the capabilities listed above, a wee beastie 
can not \emph{know}, a priori, if two \lol s are \emph{equal}. The best a 
wee beastie can know, is that a pair of \lol s have \quote{passed} all of 
the same collection of \quote{tests}. This is the concept of Bisimulation, 
see Chapter 3 of \cite{jacobs2017coalgebras}. \stopMMundi 
