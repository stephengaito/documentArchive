...
\section[title=Rigour]

\TODO{How do we program the verification engine? For (Basic) Dynamic Logic 
we build the minimal/maximal filtration (see 
\cite{demriGorankoLange2016temporalLogics} section 5.5.1 page 124; see 
also \cite{harelKozenTiuryn2000dynamicLogic} section 5.4 page 171 (for 
PDL) and section 11.4 page 297 (for first-order))(is this essentially a 
tableau?). I suspect we play the two person games on this underlying 
filtration to prove satisfiablity of $\mu$-modal logics, see 
\cite{demriGorankoLange2016temporalLogics} section 15.4 page 682 AND 
preceeding sections. How does the above inter-relate with 
\cite{harelKozenTiuryn2000dynamicLogic} deductive logic? Are these simply 
moves in the construction of the filtration? Do we extend this deductive 
logic to allow for bisimularity assertions? (See below)} 

\TODO{What is the structure of the pre and post conditions? We do NOT want 
to use meta-variables, so we use deBruijn indexing. However we may have 
complex repeated expressions, how do we identify these? Do we 
\quote{assign} them to \quote{(meta)-variables} or do we define them as 
\quote{symbols}? Use of meta-variables would allow \quote{pattern 
matching} (similar to Haskell), however this would require a unification 
engine adding to the base \joylol's complexity. I suspect meta-variables 
should be left for some future \joylol\ extension.} 

\TODO{How do we deal with identity inside a condition and between pre and 
post conditions? Clearly we have no identity, but we can assert 
bisimularity. Bisimularity \emph{between} pre and post conditions requires 
the ability to \quote{reference} both data/process/definition \lols\ in 
both pre/post \quote{states}.} 
...
are explicitly \quote{named} below, have the \quote{standard} ASCII 
...
\defineSymb{\type{isWhiteSpace?}} is defined to be: 
...
\defineSymb{\type{isStructuralCharacter?}} is defined to be: 
...
characters. Formally, \defineSymb{\type{isSymbolCharacter?}} is defined to 
be: 
...
\defineSymb{\type{isAtomicSymbol}} to be: 
...
\defineSymb{\type{symbolIsLessThan?}} to be: 
