...
booting a computer at power on, or building the first every compiler for a
programming language. Everything must be done from scratch with very few
resources.
...
techniques, has, almost certainly, not been done since sometime in late 1969
with the advent of the first self-hosting version of Unix complete with a C
compiler, see \cite[Chapter 2]{salus2008openSourceHistory}.

Bootstrapping Mathematics involves an equally tedious process. We must identify
specific mappings which are simple enough for an average mathematician to
