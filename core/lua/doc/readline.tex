...
static TextObj* createTextFromReadline(JoyLoLInterp *jInterp) {
  ...
  assert(jInterp);
  DEBUG(debugFlag, "createTextFromReadline %p\n", jInterp);
  ...
  //
  DEBUG(debugFlag, "starting readline initialization %s\n", "");
  using_history();
...
void runREPLInContext(
  ...
  while(TRUE) {
    DEBUG(debugFlag, "runREPLInContext %p reading\\n", aCtx);
    CoAlgObj* aLoL = parseOneSymbol(aText);
    DEBUG(debugFlag, "runREPLInContext %p read %p\\n", aCtx, aLoL);
    //
    if (!aLoL) break;
    //
