...
typedef void (CoAlg)(ContextObj*);

typedef struct coAlg_object_struct {
  CoAlgObj   super;
  // ??
} CoAlgObj;
...
\startCHeader
typedef CoAlgObj *(NewCoAlg)(
  JoyLoLInterp *jInterp//,
  //CFunction    *aFunc
);

#define newCoAlg(jInterp/*, aFunc*/)      \
  (                                       \
    assert(getCoAlgsClass(jInterp)    \
      ->newCoAlgFunc),                \
    (getCoAlgsClass(jInterp)          \
      ->newCoAlgFunc(jInterp/*, aFunc*/)) \
  )
\stopCHeader

\setCHeaderStream{private}
\startCHeader
extern CoAlgObj *newCoAlgImpl(
  JoyLoLInterp *jInterp//,
  //CFunction *aFunc
);
\stopCHeader
...
\startCCode
CFunctionObj *newCoAlgImpl(
  JoyLoLInterp *jInterp//,
  //CFunction *aFunc
) {
  assert(jInterp);
  CoAlgObj* aNewCoAlg =
    (CoAlgObj*)newObject(jInterp, CoAlgsTag);
  assert(aNewCoAlg);
  //aNewFunc->func  = aFunc;
  return aNewCoAlg;
}
...
\startTestCase[should create a new CoAlg]
\startCTest
