...
\startTestSuite[registerVersion]

We need a generic way of registering the git version of a given CoAlgebra. 
We do this in two parts. In the \type{bin} directory of any (collection 
of) CoAlgebra(s) there should be a shell pair of shell scripts. The 
...
git version information in the \type{gitVersion.h} file and place it into 
a \type{version} table in Lua. 
...
extern void registerVersion(lua_State *lstate, KeyValues *versionInfo);
\stopCHeader

The following implementation \emph{expects} that the C-module's table has 
already been pushed onto the Lua stack. 

\startCCode
void registerVersion(lua_State *lstate, KeyValues *versionInfo) {
  size_t numKeys = 0;
  for(numKeys = 0 ; versionInfo[numKeys].key ; numKeys++);
  lua_createtable (lstate, 0, numKeys);
  for(numKeys = 0 ; versionInfo[numKeys].key ; numKeys++) {
    lua_pushstring(lstate, versionInfo[numKeys].value);
    lua_setfield(lstate, -2, versionInfo[numKeys].key);
  }
