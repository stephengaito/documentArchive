...
Boolean loadAJoyLoLFile(
  ...
  assert(aCtx);
  assert(aCtx->jInterp);
  DEBUG(debugFlag, "loadATextFile [%s]\n", filePath);
  ...
  if (inputFile) {
    ...
    if (reportException(aCtx)) {
      DEBUG(debugFlag, "loadATextFile [%s] RAISED EXCEPTION\n", filePath);
      return FALSE;
    }
    DEBUG(debugFlag, "loadATextFile [%s] OK\n", filePath);
    return TRUE; // we have loaded this file so return TRUE;
  }
  DEBUG(debugFlag, "loadATextFile [%s] FAILED\n", filePath);
  return FALSE;
}
\stopCCode
...
Boolean loadAFileImpl(
  ...
  DEBUG(debugFlag, "loadAFile: FAILED%s\n", "");
