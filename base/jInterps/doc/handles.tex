...
\startCHeader
typedef struct pairs_data_struct {
  CoAlgHandle* car;
  CoAlgHandle* cdr;
} PairsData;

typedef struct symbols_data_struct {
  Symbol *str;
  size_t  flags;
} SymbolsData;
...
void addHandleBlock(JoyLoLInterp* jInterp) {
  ...
  for ( ; nextHandle < lastHandle ; nextHandle++ ) {
    ...
    nextHandle->data.pairs.car = NULL;
    nextHandle->data.pairs.cdr = nextHandle + 1;
  }
  nextHandle->data.pairs.cdr = jInterp->hMem.freeHandles;
...
\startCTest
  ...
  for ( ; nextHandle < lastHandle ; nextHandle++ ) {
    ...
    AssertPtrNull(nextHandle->data.pairs.car);
    AssertPtrEquals(nextHandle->data.pairs.cdr, nextHandle + 1);
  }
  AssertPtrNull(nextHandle->data.pairs.cdr);
  ...
  AssertPtrEquals(nextHandle->data.pairs.cdr, oldFreeHandles);
  AssertPtrEquals(jInterp->hMem.freeHandles,
...
void collectGarbage(JoyLoLInterp* jInterp) {
  ...
  // handle block for currently grey markings... and
  // we keep a pointer to the current handle block and
