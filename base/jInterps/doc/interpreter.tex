...
\startCHeader
typedef void *(JoyLoLCallback)(size_t);
\stopCHeader
...
typedef void (StdOutputMethod)(Symbol *aMessage);
\stopCHeader
...
\startCHeader
typedef Boolean (CoAlgInitialize)(
  JoyLoLInterp*,
  CoAlgClass*
);

typedef Boolean (CoAlgEquality)(
  CoAlgObj*,
  CoAlgObj*,
  Boolean
);
...
typedef Boolean (CoAlgPrint)(
  JoyLoLInterp*,
  StringBufferObj*,
  CoAlgObj*
);

typedef CoAlgObj* (CoAlgCopy)(
  JoyLoLInterp*,
  CoAlgObj*
);

typedef Boolean (CoAlgRelease)(
  JoyLoLInterp*,
  CoAlgObj*
);
...
typedef struct coalg_class_struct {
  Symbol          *name;
  size_t           objectSize;
  CoAlgInitialize *initializeFunc;
...
#define getCoAlgClass(jInterp, aTag)        \
  (                                         \
    assert(jInterp),                        \
    assert(aTag),                           \
...
JoyLoLInterp* newJoyLoLInterp(lua_State *lstate) {
  ...
  jInterp->writeStdOut =
    getCoreResources(JoyLoLCallback_StdOutMethod);
  jInterp->writeStdErr =
    getCoreResources(JoyLoLCallback_StdErrMethod);
  if (!(jInterp->writeStdOut) || !(jInterp->writeStdErr)) {
    fprintf(stderr, "\nERROR:\n");
    fprintf(stderr, "  Could not get the core output methods\n");
    exit(-1);
  }
...
\setCHeaderStream{private}
\startCHeader
extern Boolean initializeJInterps(
  JoyLoLInterp *jInterp,
