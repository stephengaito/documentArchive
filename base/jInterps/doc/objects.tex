...
#define isAtom(anObject) \
  ((anObject) && (anObject)->type && ((anObject)->type->isA != PairsTag))
...
\subsection[title=Allocating new objects]
...
\setCCodeStream{objects}
\startCHeader
extern CoAlgObj* newObject(JoyLoLInterp* jInterp);
\stopCHeader

\startCCode
CoAlgObj* newObject(JoyLoLInterp* jInterp) {
  assert(jInterp);
  // ensure there are some free objects
  if ( ! jInterp->hMem.freeObjects ) collectGarbage(jInterp);
  if ( ! jInterp->hMem.freeObjects ) addObjectBlock(jInterp);
  assert(jInterp->hMem.freeObjects);

  CoAlgObj* aNewObject   = jInterp->hMem.freeObjects;
  jInterp->hMem.freeObjects = (CoAlgebra*)(aNewObject->type);
...
\startCTest
  ...
  // are initialized...
  CoAlgObj* aNewObject = newObject(jInterp);
  ...
  CoAlgObj* oldFreeObjects =
    jInterp->hMem.freeObjects;
  CoAlgObj* newfreeObjects = 
    (CoAlgObj*)jInterp->hMem.freeObjects->type;

  aNewObject = newObject(jInterp);
  AssertPtrNotNull(aNewObject );
  AssertPtrNull(aNewObject->type);
  AssertIntZero(aNewObject->tag);
  AssertIntZero(aNewObject->flags);
  AssertPtrEquals(oldFreeObjects, aNewObject);
  AssertPtrEquals(newfreeObjects, jInterp->hMem.freeObjects);
\stopCTest
\stopTestCase
...
\subsection[title=Allocating new Object blocks]
...
extern void addObjectBlock(JoyLoLInterp* jInterp);
\stopCHeader


\startCCode
void addObjectBlock(JoyLoLInterp* jInterp) {
  DEBUG(FALSE, "addObjectBlock > %p %zu\n", 
...
\startCTest
  ...
  CoAlgObj* oldFreeObjects = jInterp->hMem.freeObjects;
  addObjectBlock(jInterp);
  AssertPtrNotNull(jInterp->hMem.rootObjectBlock->nextBlock);
  AssertPtrNotNull(jInterp->hMem.rootObjectBlock->nextBlock->block);
  AssertPtrNull(jInterp->hMem.rootObjectBlock->nextBlock->nextBlock);
  AssertPtrNotNull(jInterp->hMem.freeObjects);

  // check to make sure freeObjects list is correctly linked
  nextObject = jInterp->hMem.rootObjectBlock->nextBlock->block;
  lastObject = nextObject + OBJECT_BLOCK_SIZE - 1;
  for ( ; nextObject < lastObject ; nextObject++ ) {
    AssertIntTrue(IS_MEM_ALIGNED(nextObject));
    AssertIntZero(nextObject->tag);
    AssertIntZero(nextObject->flags);
  ...
  AssertPtrEquals(jInterp->hMem.freeObjects,
    jInterp->hMem.rootObjectBlock->nextBlock->block);
\stopCTest
\stopTestCase
\stopTestSuite
