...
// The following is adapted from lamma's corrected version
// at the end of his question:
//   https://stackoverflow.com/q/29287988
// as well as Joe Rossi's initial question
//   http://lua-users.org/lists/lua-l/2006-03/msg00335.html
//
static void pushLuaTo(
  lua_State  *lstate,
  ContextObj *rootCtx,
  Boolean     isData
) {
  ...
  case LUA_TTABLE : {
    ...
    for (size_t i = 1; i <= tableLen; i++) {
      ...
      pushLuaTo(lstate, rootCtx, isData);
      lua_pop(lstate, 1);
    }
  } break;
  case LUA_TFUNCTION :
...
static int lua_joylol_pushData(lua_State *lstate) {
  getJoyLoLInterpInto(lstate, jInterp);
  pushLuaTo(lstate, jInterp->rootCtx, TRUE);
  lua_pop(lstate, 1);
  return 0;
}
