...
static void initializeAllRequired(
  ...
  for( size_t i = 1; i < NumRequiredCoAlgs; i++) {
    ...
    if (aCoAlg) {
      if (jInterp->verbose) 
        printf("    initializing [joylol.%s]\n", aCoAlg->name);
      aCoAlg->initializeFunc(jInterp, aCoAlg);
      if (jInterp->verbose) 
        printf("    initialized [joylol.%s]\n", aCoAlg->name);
    }
  }
