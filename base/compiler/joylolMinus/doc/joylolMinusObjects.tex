...
Symbol *appendSymbols(Symbol *sym1, Symbol *sym2);
...
#ifndef NDEBUG
#include <stdio.h>
#define setDebugging(aCtx, aBool)  \
  assert(aCtx);                    \
  aCtx->debug = aBool
#define getDebuggingInto(aCtx, aVar) \
  assert(aCtx);                      \
  Boolean aVar = aCtx->debug
#define DEBUG(aCtx, format, ... )  \
  assert(aCtx);                    \
  if (aCtx->debug) {               \
    printf(format, __VA_ARGS__ );  \
  }
#else
#define startDebugging(aCtx)
#define stopDebuggging(aCtx)
#define DEBUG(aCtx, format, ...)
#endif
...
void contextPrinter(JObj *anObj, Symbol *indent, size_t timeToLive) {
  ...
  printf("%s<data>\n", indent);
  printJObj(asCtxData(anObj), indent, 20);
  printf("%s<process>\n", indent);
  printJObj(asCtxProcess(anObj), indent, 20);
}
\stopCCode
