...
static void defineAP(ContextObj* aCtx) {
  ...
  defineJoyLoLInRoot(aCtx->jInterp, asSymbol(name), definition);
}
\stopCCode
...
static void defineContextAP(ContextObj* aCtx) {
  ...
  assert(newCtx);
  defineContextInRoot(jInterp, asSymbol(contextName), newCtx);
}
\stopCCode
...
static void defineNamingAP(ContextObj* aCtx) {
  ...
  popCtxDataInto(aCtx, dictionaryName);
  if (!isSymbol(dictionaryName)) {
    raiseException(aCtx,
      "defineNaming required a symbol as top");
    return;
  }
  DictObj* newDict =
    newDictionary(jInterp, aCtx->dict);
  assert(newDict);
  defineNamingInRoot(jInterp, asSymbol(dictionaryName), newDict);
}
\stopCCode
